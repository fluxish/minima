...
class Validation
{
        ...
        foreach($this->_fields as $field_name=>$field)
        {
            foreach($field['errors'] as $err){
                if(empty($field['rules'][$err])){
                    $errors[$field['name']][] = sprintf($language->item('validation_'.$err), 
                        $field['label']);
                }
                else{
                    $errors[$field['name']][] = sprintf($language->item('validation_'.$err), 
                        $field['label'], $field['rules'][$err]);
                }
            }    
        }
    ...
    {
        $value = (string)$value;
        if(strlen($value) < $lim) return false;
        return true;
    }
    ...
    {
        $value = (string)$value;
        if(strlen($value) == $lim) return true;
        return false;
    }
    ...
    {
        if((float)$value < (float)$lim) return false;
        return true;
    }
    ...
    {
        if((float)$value == (float)$lim) return true;
        return false;
    }
    ...
    {
        if(preg_match('/^([A-Za-z\ ])+$/',$str) == 0) return false;
        return true;
    }
    ...
    {
        if(preg_match('/^([A-Za-z0-9\ ])+$/',$str) == 0) return false;
        return true;
    }
    ...
    {
        if(preg_match('/^([A-Za-z0-9_.\-\ ])+$/',$str) == 0) return false;
        return true;
    }    
    ...
    {
        if(preg_match('/^([+\-]?[0-9]+\.?[0-9]*)$/',$str) == 0) return false;
        return true;
    }
    ...
    {
        if(preg_match('/^([+\-]?[0-9]+)$/',$str) == 0) return false;
        return true;
    }
    ...
    public function natural_no_zero($str)
    {
        ...
        else if(preg_match('/^([0-9]+)$/',$str) == 0) return false;
        return true;
    }
